...
function render() {
  ...
  const dashboardRoutes = ['/dashboard', '/broker', '/builder', '/strategies', '/backtest', '/simulator', '/reports', '/subscription', '/wallet', '/livebot'];
...
function renderDashboard(route) {
  const sidebarLinks = [
    ...
    { path: '/livebot', icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>`, label: 'Live Bot' },
  ];
  ...
  switch (route) {
    ...
    case '/livebot': renderLiveBotPage(main); break;
...
// ═══════════════════════════════════════════════════════════════
// LIVE BOT — Nifty 50 Options Trading Strategy Engine
// Ported from uncle's Colab notebook, all 15 bugs fixed
// ═══════════════════════════════════════════════════════════════
...
const bot = {
  ...
  // Strategy config
  config: {
    firstCandleStartTime: '09:15',
    firstCandleEndTime: '09:18',
    profitEntryThreshold: 0.07,
    optionStrikeDiff: 50,
    marketCloseTime: '15:15',
    quantityPerLot: 50,
    dailyMaxLoss: 1000,
    dynamicProfitLockEnabled: true,
    initialProfitLockTrigger: 600,
    profitLockBuffer: 100,
    productType: 'MIS',
  },
  ...
  niftyPrice: 24000,
  priceHistory: [],
  firstCandle: null, // {open, high, low, close}
  initialSpot: 0,
  ...
  sellLeg: null,
  sellLegSLHit: false,
  reEntryMark: { low: 0, high: 0, candleType: null },
  ...
  // Simulated candles (3-min)
  candles: [],
  currentCandle: null,
  candleTickCounter: 0,
  TICKS_PER_CANDLE: 18, // 18 ticks × 10s = 180s = 3 min in sim time
...
function botLog(msg, level = 'INFO') {
  const ts = `${String(bot.simHour).padStart(2,'0')}:${String(bot.simMinute).padStart(2,'0')}:${String(bot.simSecond).padStart(2,'0')}`;
  bot.eventLog.unshift({ ts, level, msg });
  if (bot.eventLog.length > 200) bot.eventLog.length = 200;
}
...
function simTimeStr() {
  return `${String(bot.simHour).padStart(2,'0')}:${String(bot.simMinute).padStart(2,'0')}`;
}

function simTimePast(hhmm) {
  const [h, m] = hhmm.split(':').map(Number);
  return bot.simHour > h || (bot.simHour === h && bot.simMinute >= m);
}

// ── Price Simulation (random walk with mean reversion) ──
function simulatePrice() {
  const drift = (24000 - bot.niftyPrice) * 0.0003; // Mean reversion
  const volatility = (Math.random() - 0.5) * 12; // ±6 pts per tick
  const momentum = bot.priceHistory.length > 1
    ? (bot.priceHistory[bot.priceHistory.length - 1] - bot.priceHistory[bot.priceHistory.length - 2]) * 0.1
    : 0;
  bot.niftyPrice = Math.round((bot.niftyPrice + drift + volatility + momentum) * 100) / 100;
  bot.priceHistory.push(bot.niftyPrice);
  if (bot.priceHistory.length > 500) bot.priceHistory.shift();
}

// ── Candle Management ──
function updateCandle() {
  if (!bot.currentCandle) {
    bot.currentCandle = {
      open: bot.niftyPrice,
      high: bot.niftyPrice,
      low: bot.niftyPrice,
      close: bot.niftyPrice,
      time: simTimeStr(),
    };
    bot.candleTickCounter = 0;
  }

  bot.currentCandle.high = Math.max(bot.currentCandle.high, bot.niftyPrice);
  bot.currentCandle.low = Math.min(bot.currentCandle.low, bot.niftyPrice);
  bot.currentCandle.close = bot.niftyPrice;
  bot.candleTickCounter++;

  if (bot.candleTickCounter >= bot.TICKS_PER_CANDLE) {
    bot.candles.push({ ...bot.currentCandle });
    if (bot.candles.length > 100) bot.candles.shift();
    const finished = { ...bot.currentCandle };
    bot.currentCandle = null;
    return finished;
  }
  return null;
}
...
function simOptionPrice(isCall, strike, spot) {
  const intrinsic = isCall ? Math.max(0, spot - strike) : Math.max(0, strike - spot);
  const extrinsic = 40 + Math.random() * 30;
...
function getNearestExpiry() {
  ...
  let daysToThur = (4 - day + 7) % 7; // 4 = Thursday
  if (daysToThur === 0) daysToThur = 0; // Today is Thursday, use it
  const expiry = new Date(today);
  expiry.setDate(today.getDate() + daysToThur);
  return expiry;
}

function formatExpiry(date) {
  const months = ['JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'];
  return `${String(date.getFullYear()).slice(-2)}${months[date.getMonth()]}${String(date.getDate()).padStart(2,'0')}`;
}

// ── Order Placement ──
function placeOrder(symbol, type, qty, price) {
  const order = {
  ...
  bot.orders.unshift(order);
  if (bot.orders.length > 50) bot.orders.length = 50;
  botLog(`${type} ${qty}x ${symbol} @ ₹${order.price}`, 'ORDER');
  return order;
}

// ── Enter Spread ──
function enterSpread(spreadType) {
  if (bot.tradeInitiated) {
    botLog('Trade already initiated, skipping.', 'WARN');
    return false;
  }

  const spot = bot.niftyPrice;
  const atm = getATMStrike(spot);
  const expiry = formatExpiry(getNearestExpiry());
  const diff = bot.config.optionStrikeDiff;
  const qty = bot.config.quantityPerLot;
  const isCall = spreadType === 'CALL_SPREAD';
  const optType = isCall ? 'CE' : 'PE';

  const buyStrike = isCall ? atm + diff : atm - diff;
  const sellStrike = isCall ? atm - diff : atm + diff;

  const buySymbol = `NIFTY${expiry}${buyStrike}${optType}`;
  const sellSymbol = `NIFTY${expiry}${sellStrike}${optType}`;

  const buyPrice = simOptionPrice(isCall, buyStrike, spot);
  const sellPrice = simOptionPrice(isCall, sellStrike, spot);

  const buyOrder = placeOrder(buySymbol, 'BUY', qty, buyPrice);
  const sellOrder = placeOrder(sellSymbol, 'SELL', qty, sellPrice);
  ...
  bot.sellLeg = {
    ...
    isCall,
    slPrice: isCall ? bot.firstCandle.high : bot.firstCandle.low,
  };

  bot.sellLegSLHit = false;
  bot.reEntryMark = { low: 0, high: 0, candleType: null };
  bot.tradeInitiated = true;
  bot.spreadType = spreadType;

  botLog(`Entered ${spreadType}: Buy ${buySymbol} @ ₹${buyPrice}, Sell ${sellSymbol} @ ₹${sellPrice}`, 'TRADE');
  botLog(`Sell leg SL set at ₹${bot.sellLeg.slPrice.toFixed(2)}`, 'TRADE');
  return true;
}
...
function updateLegPrices() {
  if (bot.buyLeg) {
    bot.buyLeg.currentPrice = simOptionPrice(bot.buyLeg.isCall, bot.buyLeg.strike, bot.niftyPrice);
  }
  if (bot.sellLeg && !bot.sellLegSLHit) {
    bot.sellLeg.currentPrice = simOptionPrice(bot.sellLeg.isCall, bot.sellLeg.strike, bot.niftyPrice);
  }
}
...
function calculatePnl() {
  let unrealized = 0;
  if (bot.buyLeg) {
    unrealized += (bot.buyLeg.currentPrice - bot.buyLeg.entryPrice) * bot.buyLeg.qty;
  }
  if (bot.sellLeg && !bot.sellLegSLHit) {
    unrealized += (bot.sellLeg.entryPrice - bot.sellLeg.currentPrice) * bot.sellLeg.qty;
  }
  bot.unrealizedPnl = Math.round(unrealized * 100) / 100;
}

function getNetPnl() {
  return Math.round((bot.realizedPnl + bot.unrealizedPnl) * 100) / 100;
}

// ── Square off all ──
function squareOffAll(reason) {
  botLog(`SQUARING OFF ALL — ${reason}`, 'WARN');

  let realized = 0;
  if (bot.buyLeg) {
    const exitPrice = bot.buyLeg.currentPrice;
    placeOrder(bot.buyLeg.symbol, 'SELL', bot.buyLeg.qty, exitPrice);
    realized += (exitPrice - bot.buyLeg.entryPrice) * bot.buyLeg.qty;
    bot.buyLeg = null;
  }
  if (bot.sellLeg && !bot.sellLegSLHit) {
    const exitPrice = bot.sellLeg.currentPrice;
    placeOrder(bot.sellLeg.symbol, 'BUY', bot.sellLeg.qty, exitPrice);
    realized += (bot.sellLeg.entryPrice - exitPrice) * bot.sellLeg.qty;
    bot.sellLeg = null;
  }

  bot.realizedPnl += Math.round(realized * 100) / 100;
  bot.unrealizedPnl = 0;
  bot.tradeInitiated = false;
  bot.sellLegSLHit = false;
  botLog(`Positions closed. Realized from close: ₹${realized.toFixed(2)}`, 'TRADE');
}

// ── Check SL ──
function checkSellLegSL(candle) {
  if (!bot.sellLeg || bot.sellLegSLHit) return false;

  let triggered = false;
  if (bot.spreadType === 'CALL_SPREAD' && candle.close > bot.sellLeg.slPrice) {
    botLog(`CALL SL HIT! Close ${candle.close.toFixed(2)} > SL ${bot.sellLeg.slPrice.toFixed(2)}`, 'WARN');
    triggered = true;
  } else if (bot.spreadType === 'PUT_SPREAD' && candle.close < bot.sellLeg.slPrice) {
    botLog(`PUT SL HIT! Close ${candle.close.toFixed(2)} < SL ${bot.sellLeg.slPrice.toFixed(2)}`, 'WARN');
    triggered = true;
  }
  ...
  if (triggered) {
    const exitPrice = bot.sellLeg.currentPrice;
    placeOrder(bot.sellLeg.symbol, 'BUY', bot.sellLeg.qty, exitPrice);
    const slRealized = (bot.sellLeg.entryPrice - exitPrice) * bot.sellLeg.qty;
    bot.realizedPnl += Math.round(slRealized * 100) / 100;
    botLog(`Sell leg closed at ₹${exitPrice.toFixed(2)}. P&L: ₹${slRealized.toFixed(2)}`, 'TRADE');
    bot.sellLegSLHit = true;
    bot.reEntryMark = { low: 0, high: 0, candleType: null };
    return true;
  }
  return false;
...
// ── Re-entry ──
function attemptReEntry(candle) {
  if (!bot.sellLegSLHit || !bot.buyLeg) return false;

  const rm = bot.reEntryMark;

  if (bot.spreadType === 'CALL_SPREAD') {
    if (rm.candleType === null) {
      if (candle.close < candle.open) { // Red candle
        rm.low = candle.low;
        rm.high = candle.high;
        rm.candleType = 'RED';
        botLog(`[RE-ENTRY] Red candle marked: L=${candle.low.toFixed(2)} H=${candle.high.toFixed(2)}`, 'INFO');
      }
    } else if (rm.candleType === 'RED') {
      if (candle.close < rm.low) {
        botLog(`[RE-ENTRY] Confirmed! Close ${candle.close.toFixed(2)} < Red Low ${rm.low.toFixed(2)}`, 'TRADE');
        return executeReEntry('CE', rm.high);
      } else if (candle.close > rm.high) {
        botLog(`[RE-ENTRY] Discarded. Close ${candle.close.toFixed(2)} > Red High ${rm.high.toFixed(2)}`, 'INFO');
        rm.candleType = null;
      }
    }
  } else if (bot.spreadType === 'PUT_SPREAD') {
    if (rm.candleType === null) {
      if (candle.close > candle.open) { // Green candle
        rm.low = candle.low;
        rm.high = candle.high;
        rm.candleType = 'GREEN';
        botLog(`[RE-ENTRY] Green candle marked: L=${candle.low.toFixed(2)} H=${candle.high.toFixed(2)}`, 'INFO');
      }
    } else if (rm.candleType === 'GREEN') {
      if (candle.close > rm.high) {
        botLog(`[RE-ENTRY] Confirmed! Close ${candle.close.toFixed(2)} > Green High ${rm.high.toFixed(2)}`, 'TRADE');
        return executeReEntry('PE', rm.low);
      } else if (candle.close < rm.low) {
        botLog(`[RE-ENTRY] Discarded. Close ${candle.close.toFixed(2)} < Green Low ${rm.low.toFixed(2)}`, 'INFO');
        rm.candleType = null;
      }
    }
  }
...
function executeReEntry(optType, newSL) {
  const spot = bot.niftyPrice;
  const atm = getATMStrike(spot);
  const expiry = formatExpiry(getNearestExpiry());
  const diff = bot.config.optionStrikeDiff;
  ...
  const order = placeOrder(sellSymbol, 'SELL', bot.config.quantityPerLot, sellPrice);
  ...
  bot.sellLegSLHit = false;
  bot.reEntryMark = { low: 0, high: 0, candleType: null };
  botLog(`Re-entered sell leg: ${sellSymbol} @ ₹${sellPrice}. New SL: ₹${newSL.toFixed(2)}`, 'TRADE');
  return true;
}

// ── Dynamic Profit Lock ──
function updateProfitLock() {
  if (!bot.config.dynamicProfitLockEnabled) return;
  const netPnl = getNetPnl();
  if (netPnl >= bot.config.initialProfitLockTrigger) {
    const newLock = netPnl - bot.config.profitLockBuffer;
    if (newLock > bot.currentLockedProfit) {
      bot.currentLockedProfit = newLock;
      botLog(`Profit lock updated → ₹${bot.currentLockedProfit.toFixed(2)}`, 'INFO');
    }
  }
}

function checkProfitLock() {
  if (!bot.config.dynamicProfitLockEnabled || bot.currentLockedProfit <= 0) return false;
  const netPnl = getNetPnl();
  if (netPnl <= bot.currentLockedProfit) {
    botLog(`PROFIT LOCK triggered! P&L ₹${netPnl.toFixed(2)} ≤ Lock ₹${bot.currentLockedProfit.toFixed(2)}`, 'WARN');
    squareOffAll('Dynamic Profit Lock triggered');
    bot.currentLockedProfit = 0;
    return true;
  }
  return false;
}
...
function botTick() {
  ...
  if (bot.killSwitchActive) {
    if (bot.tradeInitiated) squareOffAll('EMERGENCY KILL SWITCH');
    stopBot('Kill switch activated');
    return;
  }

  // Market close
  if (simTimePast(bot.config.marketCloseTime)) {
    if (bot.tradeInitiated) squareOffAll('Market close (15:15)');
    bot.marketClosed = true;
    stopBot('Market closed for the day');
    return;
  }
  ...
  // Build candle
  const finishedCandle = updateCandle();
  ...
  // Daily max loss
  if (getNetPnl() < -bot.config.dailyMaxLoss) {
    botLog(`DAILY MAX LOSS HIT! P&L: ₹${getNetPnl().toFixed(2)}`, 'ERROR');
    if (bot.tradeInitiated) squareOffAll('Daily max loss reached');
    bot.dailyMaxLossHit = true;
    stopBot('Daily max loss reached');
    return;
  }

  // Phase 1: First candle capture (9:15–9:18)
  if (!bot.firstCandle && simTimePast('09:18') && bot.candles.length > 0) {
    bot.firstCandle = { ...bot.candles[0] };
    bot.initialSpot = bot.firstCandle.open;
    botLog(`First candle captured: O=${bot.firstCandle.open.toFixed(2)} H=${bot.firstCandle.high.toFixed(2)} L=${bot.firstCandle.low.toFixed(2)} C=${bot.firstCandle.close.toFixed(2)}`, 'INFO');
  }

  // Phase 2: Entry
  if (bot.firstCandle && !bot.tradeInitiated && simTimePast('09:18')) {
    const pctChange = ((bot.niftyPrice - bot.initialSpot) / bot.initialSpot) * 100;
    if (pctChange > bot.config.profitEntryThreshold) {
      botLog(`Nifty +${pctChange.toFixed(3)}% → Entering PUT SPREAD`, 'TRADE');
      enterSpread('PUT_SPREAD');
    } else if (pctChange < -bot.config.profitEntryThreshold) {
      botLog(`Nifty ${pctChange.toFixed(3)}% → Entering CALL SPREAD`, 'TRADE');
      enterSpread('CALL_SPREAD');
    }
  }

  // Phase 3: Manage (on candle close)
  if (bot.tradeInitiated && finishedCandle) {
    if (!bot.sellLegSLHit) {
      checkSellLegSL(finishedCandle);
    } else {
      attemptReEntry(finishedCandle);
    }
    updateProfitLock();
    checkProfitLock();
  }
...
function startBot() {
  ...
  // Reset state
  bot.niftyPrice = 23950 + (Math.random() - 0.5) * 100;
  bot.priceHistory = [bot.niftyPrice];
  bot.firstCandle = null;
  bot.initialSpot = 0;
  bot.tradeInitiated = false;
  bot.spreadType = null;
  bot.buyLeg = null;
  ...
  bot.reEntryMark = { low: 0, high: 0, candleType: null };
  bot.currentLockedProfit = 0;
  bot.killSwitchActive = false;
  bot.dailyMaxLossHit = false;
  bot.marketClosed = false;
  bot.realizedPnl = 0;
  bot.unrealizedPnl = 0;
  bot.orders = [];
  bot.eventLog = [];
  ...
  bot.running = true;
  botLog('Bot started — Paper Trading Mode', 'INFO');
  botLog(`Config: Entry threshold ±${bot.config.profitEntryThreshold}%, Max Loss ₹${bot.config.dailyMaxLoss}, Lot size ${bot.config.quantityPerLot}`, 'INFO');

  bot.intervalId = setInterval(botTick, bot._simSpeed || 1000);
  updateLiveBotUI();
}
...
function stopBot(reason = 'Manual stop') {
  ...
  botLog(`Bot stopped — ${reason}`, 'WARN');
  updateLiveBotUI();
}
...
window.activateKillSwitch = function() {
  bot.killSwitchActive = true;
  botLog('KILL SWITCH ACTIVATED!', 'ERROR');
};
...
function drawPriceChart() {
  ...
  const data = bot.priceHistory;
  if (data.length < 2) return;

  const visibleCount = Math.min(data.length, 200);
  const slice = data.slice(-visibleCount);
  const min = Math.min(...slice) - 5;
  const max = Math.max(...slice) + 5;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const y = (H / 4) * i + 4;
    ctx.fillText(val.toFixed(0), W - 4, y + 10);
  }

  // First candle line
  if (bot.firstCandle) {
    const openY = H - ((bot.firstCandle.open - min) / range) * H;
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`Open ${bot.firstCandle.open.toFixed(0)}`, 4, openY - 4);
  }

  // SL line
  if (bot.sellLeg && !bot.sellLegSLHit) {
    const slY = H - ((bot.sellLeg.slPrice - min) / range) * H;
    ...
    ctx.font = '9px JetBrains Mono, monospace';
    ctx.textAlign = 'left';
    ctx.fillText(`SL ${bot.sellLeg.slPrice.toFixed(0)}`, 4, slY - 4);
  }

  // Price line gradient
  const grad = ctx.createLinearGradient(0, 0, 0, H);
  const lastPrice = slice[slice.length - 1];
  const firstPrice = slice[0];
  const isUp = lastPrice >= firstPrice;
  if (isUp) {
    grad.addColorStop(0, 'rgba(0,230,118,0.15)');
    grad.addColorStop(1, 'rgba(0,230,118,0)');
  } else {
    grad.addColorStop(0, 'rgba(255,71,87,0.15)');
  ...
  for (let i = 0; i < slice.length; i++) {
    const x = (i / (slice.length - 1)) * (W - 40);
    const y = H - ((slice[i] - min) / range) * H;
    if (i === 0) ctx.moveTo(x, y);
    else ctx.lineTo(x, y);
  }
  ctx.lineTo((slice.length - 1) / (slice.length - 1) * (W - 40), H);
  ctx.lineTo(0, H);
  ctx.closePath();
  ctx.fillStyle = grad;
  ctx.fill();
  ...
  ctx.textAlign = 'left';
  ctx.fillText(`₹${lastPrice.toFixed(2)}`, lastX + 6, lastY + 4);
}

// ── UI Update ──
function updateLiveBotUI() {
  // Status badge
  const statusEl = document.getElementById('lb-status');
  if (statusEl) {
    if (bot.running) {
      statusEl.className = 'lb-status-badge running';
      statusEl.innerHTML = '<span class="lb-pulse"></span> Running';
    } else if (bot.killSwitchActive) {
      statusEl.className = 'lb-status-badge killed';
      statusEl.textContent = '⚡ Kill Switch';
    } else if (bot.dailyMaxLossHit) {
      statusEl.className = 'lb-status-badge stopped';
      statusEl.textContent = '🛑 Max Loss';
    } else if (bot.marketClosed) {
      statusEl.className = 'lb-status-badge stopped';
      statusEl.textContent = '🔒 Market Closed';
    } else {
      statusEl.className = 'lb-status-badge idle';
      statusEl.textContent = '⏸ Idle';
    }
  }

  // Sim time
  const timeEl = document.getElementById('lb-simtime');
  if (timeEl) timeEl.textContent = simTimeStr();

  // Price
  const priceEl = document.getElementById('lb-price');
  if (priceEl) priceEl.textContent = `₹${bot.niftyPrice.toFixed(2)}`;
  ...
  const setVal = (id, val) => {
    ...
    if (el) {
      ...
      el.className = val >= 0 ? 'lb-pnl-val profit' : 'lb-pnl-val loss';
    }
  };
  setVal('lb-net-pnl', netPnl);
  setVal('lb-realized', bot.realizedPnl);
  setVal('lb-unrealized', bot.unrealizedPnl);

  // Locked profit
  const lockEl = document.getElementById('lb-lock');
  if (lockEl) lockEl.textContent = bot.currentLockedProfit > 0 ? `₹${bot.currentLockedProfit.toFixed(2)}` : '—';

  // Spread type
  const spreadEl = document.getElementById('lb-spread');
  if (spreadEl) spreadEl.textContent = bot.spreadType || 'Waiting...';

  // First candle
  const fcEl = document.getElementById('lb-fc');
  if (fcEl) {
    if (bot.firstCandle) {
      fcEl.textContent = `O:${bot.firstCandle.open.toFixed(0)} H:${bot.firstCandle.high.toFixed(0)} L:${bot.firstCandle.low.toFixed(0)} C:${bot.firstCandle.close.toFixed(0)}`;
    } else {
      fcEl.textContent = 'Waiting for 09:18...';
    }
  }

  // Positions table
  const posEl = document.getElementById('lb-positions');
  if (posEl) {
    let rows = '';
    if (bot.buyLeg) {
      const pnl = (bot.buyLeg.currentPrice - bot.buyLeg.entryPrice) * bot.buyLeg.qty;
      rows += `<tr>
        <td><span class="lb-tag buy">BUY</span></td>
        <td>${bot.buyLeg.symbol}</td>
        <td>${bot.buyLeg.qty}</td>
        <td>₹${bot.buyLeg.entryPrice.toFixed(2)}</td>
        <td>₹${bot.buyLeg.currentPrice.toFixed(2)}</td>
        <td class="${pnl >= 0 ? 'profit' : 'loss'}">₹${pnl.toFixed(2)}</td>
      </tr>`;
    }
    if (bot.sellLeg && !bot.sellLegSLHit) {
      const pnl = (bot.sellLeg.entryPrice - bot.sellLeg.currentPrice) * bot.sellLeg.qty;
      rows += `<tr>
        <td><span class="lb-tag sell">SELL</span></td>
        <td>${bot.sellLeg.symbol}</td>
        <td>${bot.sellLeg.qty}</td>
        <td>₹${bot.sellLeg.entryPrice.toFixed(2)}</td>
        <td>₹${bot.sellLeg.currentPrice.toFixed(2)}</td>
        <td class="${pnl >= 0 ? 'profit' : 'loss'}">₹${pnl.toFixed(2)}</td>
      </tr>`;
    }
    if (!rows) rows = '<tr><td colspan="6" class="lb-empty">No active positions</td></tr>';
    posEl.innerHTML = rows;
  }

  // Orders table
  const ordEl = document.getElementById('lb-orders');
  if (ordEl) {
    const recent = bot.orders.slice(0, 8);
    ordEl.innerHTML = recent.length ? recent.map(o => `<tr>
      <td class="lb-mono">${o.time}</td>
      <td><span class="lb-tag ${o.type.toLowerCase()}">${o.type}</span></td>
      <td>${o.symbol}</td>
      <td>${o.qty}</td>
      <td>₹${o.price.toFixed(2)}</td>
      <td><span class="lb-status-pill">${o.status}</span></td>
    </tr>`).join('') : '<tr><td colspan="6" class="lb-empty">No orders yet</td></tr>';
  }

  // Event log
  const logEl = document.getElementById('lb-log');
  if (logEl) {
    const recent = bot.eventLog.slice(0, 40);
    logEl.innerHTML = recent.map(e => {
      const cls = e.level === 'ERROR' ? 'error' : e.level === 'WARN' ? 'warn' : e.level === 'TRADE' ? 'trade' : e.level === 'ORDER' ? 'order' : 'info';
      return `<div class="lb-log-line ${cls}"><span class="lb-log-ts">${e.ts}</span><span class="lb-log-lvl">${e.level}</span>${e.msg}</div>`;
    }).join('');
  }
  ...
  const killBtn = document.getElementById('lb-kill-btn');
  if (startBtn) startBtn.disabled = bot.running;
  if (stopBtn) stopBtn.disabled = !bot.running;
  if (killBtn) killBtn.disabled = !bot.running;
}

// ── Render Page ──
function renderLiveBotPage(el) {
  ...
  el.innerHTML = `
    <div class="lb-page">
      <div class="lb-header">
        <div>
          <h1 class="page-title">Live Trading Bot</h1>
          <p class="page-subtitle">Nifty 50 Options · Paper Trading Mode</p>
        </div>
        <div class="lb-header-right">
          <div id="lb-status" class="lb-status-badge idle">⏸ Idle</div>
          <div class="lb-simtime-box">
            <span class="lb-simtime-label">SIM TIME</span>
            <span id="lb-simtime" class="lb-simtime-val">09:15</span>
          </div>
        </div>
      </div>
      ...
      <div class="lb-controls">
        <button id="lb-start-btn" class="btn btn-primary" onclick="startBot()">▶ Start Bot</button>
        <button id="lb-stop-btn" class="btn btn-outline" onclick="stopBot()" disabled>⏹ Stop Bot</button>
        <button id="lb-kill-btn" class="lb-kill-btn" onclick="if(confirm('EMERGENCY: Square off ALL positions and stop?')) activateKillSwitch()" disabled>⚡ KILL SWITCH</button>
        <div class="lb-price-display">
          <span class="lb-price-label">NIFTY 50</span>
          <span id="lb-price" class="lb-price-val">₹24,000.00</span>
        </div>
      </div>

      <!-- P&L Row -->
      <div class="lb-pnl-row">
        <div class="lb-pnl-card">
          <div class="lb-pnl-label">Net P&L</div>
          <div id="lb-net-pnl" class="lb-pnl-val">₹0.00</div>
        </div>
        <div class="lb-pnl-card">
          <div class="lb-pnl-label">Realized</div>
          <div id="lb-realized" class="lb-pnl-val">₹0.00</div>
        </div>
        <div class="lb-pnl-card">
          <div class="lb-pnl-label">Unrealized</div>
          <div id="lb-unrealized" class="lb-pnl-val">₹0.00</div>
        </div>
        <div class="lb-pnl-card">
          <div class="lb-pnl-label">Profit Lock</div>
          <div id="lb-lock" class="lb-pnl-val">—</div>
        </div>
      </div>
      ...
      <div class="lb-info-row">
        <div class="lb-info-card">
          <div class="lb-info-label">Spread Type</div>
          <div id="lb-spread" class="lb-info-val">Waiting...</div>
        </div>
        <div class="lb-info-card">
          <div class="lb-info-label">First Candle (9:15–9:18)</div>
          <div id="lb-fc" class="lb-info-val lb-mono">Waiting for 09:18...</div>
        </div>
        <div class="lb-info-card">
          <div class="lb-info-label">Entry Threshold</div>
          <div class="lb-info-val">±${bot.config.profitEntryThreshold}%</div>
        </div>
        <div class="lb-info-card">
          <div class="lb-info-label">Max Loss / Day</div>
          <div class="lb-info-val">₹${bot.config.dailyMaxLoss}</div>
        </div>
      </div>

      <!-- Chart -->
      <div class="lb-chart-section">
        <div class="lb-section-header">Nifty 50 Price · Simulated</div>
        <canvas id="lb-chart" class="lb-chart-canvas"></canvas>
      </div>

      <!-- Positions -->
      <div class="lb-section">
        <div class="lb-section-header">Active Positions</div>
        <div class="lb-table-wrap">
          <table class="lb-table">
            <thead><tr><th>Side</th><th>Symbol</th><th>Qty</th><th>Entry</th><th>LTP</th><th>P&L</th></tr></thead>
            <tbody id="lb-positions"><tr><td colspan="6" class="lb-empty">No active positions</td></tr></tbody>
          </table>
        </div>
      </div>
      ...
      <!-- Orders -->
      <div class="lb-section">
        <div class="lb-section-header">Order Book</div>
        <div class="lb-table-wrap">
          <table class="lb-table">
            <thead><tr><th>Time</th><th>Side</th><th>Symbol</th><th>Qty</th><th>Price</th><th>Status</th></tr></thead>
            <tbody id="lb-orders"><tr><td colspan="6" class="lb-empty">No orders yet</td></tr></tbody>
          </table>
        </div>
      </div>
      ...
      <!-- Event Log -->
      <div class="lb-section">
        <div class="lb-section-header">Event Log</div>
        <div id="lb-log" class="lb-log"></div>
      </div>

      <!-- Config (Editable) -->
      <div class="lb-section">
        <div class="lb-section-header" style="display:flex;justify-content:space-between;align-items:center;">
          <span>⚙ Strategy Configuration</span>
          <span class="lb-config-hint">Edit values below, then click Save</span>
        </div>
        <div class="lb-config-edit-grid">

          <div class="lb-cfg-field">
            <label for="cfg-product">Product Type</label>
            <select id="cfg-product" class="lb-input">
              <option value="MIS" ${bot.config.productType === 'MIS' ? 'selected' : ''}>MIS (Intraday)</option>
              <option value="NRML" ${bot.config.productType === 'NRML' ? 'selected' : ''}>NRML (Carry Forward)</option>
            </select>
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-lot">Lot Size</label>
            <div class="lb-input-group">
              <input id="cfg-lot" type="number" class="lb-input" value="${bot.config.quantityPerLot}" min="1" max="500" step="1">
              <span class="lb-input-suffix">qty</span>
            </div>
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-strike">Strike Difference</label>
            <div class="lb-input-group">
              <input id="cfg-strike" type="number" class="lb-input" value="${bot.config.optionStrikeDiff}" min="50" max="500" step="50">
              <span class="lb-input-suffix">pts</span>
            </div>
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-entry">Entry Threshold</label>
            <div class="lb-input-group">
              <span class="lb-input-prefix">±</span>
              <input id="cfg-entry" type="number" class="lb-input" value="${bot.config.profitEntryThreshold}" min="0.01" max="2" step="0.01">
              <span class="lb-input-suffix">%</span>
            </div>
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-maxloss">Max Daily Loss</label>
            <div class="lb-input-group">
              <span class="lb-input-prefix">₹</span>
              <input id="cfg-maxloss" type="number" class="lb-input" value="${bot.config.dailyMaxLoss}" min="100" max="50000" step="100">
            </div>
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-locktrigger">Profit Lock Trigger</label>
            <div class="lb-input-group">
              <span class="lb-input-prefix">₹</span>
              <input id="cfg-locktrigger" type="number" class="lb-input" value="${bot.config.initialProfitLockTrigger}" min="100" max="50000" step="50">
            </div>
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-lockbuf">Profit Lock Buffer</label>
            <div class="lb-input-group">
              <span class="lb-input-prefix">₹</span>
              <input id="cfg-lockbuf" type="number" class="lb-input" value="${bot.config.profitLockBuffer}" min="10" max="5000" step="10">
            </div>
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-close">Auto Square-off</label>
            <input id="cfg-close" type="time" class="lb-input" value="${bot.config.marketCloseTime}">
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-candlestart">Candle Start</label>
            <input id="cfg-candlestart" type="time" class="lb-input" value="${bot.config.firstCandleStartTime}">
          </div>

          <div class="lb-cfg-field">
            <label for="cfg-candleend">Candle End</label>
            <input id="cfg-candleend" type="time" class="lb-input" value="${bot.config.firstCandleEndTime}">
          </div>

          <div class="lb-cfg-field lb-cfg-toggle-field">
            <label>Dynamic Profit Lock</label>
            <label class="lb-toggle">
              <input type="checkbox" id="cfg-profitlock" ${bot.config.dynamicProfitLockEnabled ? 'checked' : ''}>
              <span class="lb-toggle-slider"></span>
              <span class="lb-toggle-text" id="cfg-profitlock-text">${bot.config.dynamicProfitLockEnabled ? 'Enabled' : 'Disabled'}</span>
            </label>
          </div>

          <div class="lb-cfg-field lb-cfg-speed-field">
            <label for="cfg-speed">Sim Speed</label>
            <select id="cfg-speed" class="lb-input">
              <option value="2000">0.5x (Slow)</option>
              <option value="1000" selected>1x (Normal)</option>
              <option value="500">2x (Fast)</option>
              <option value="200">5x (Turbo)</option>
              <option value="50">20x (Warp)</option>
            </select>
          </div>
        ...
        <div class="lb-config-actions">
          <button class="btn btn-primary" onclick="applyBotConfig()">💾 Save Configuration</button>
          <button class="btn btn-outline" onclick="resetBotConfig()">↺ Reset to Defaults</button>
        </div>
      </div>
    </div>
  `;

  // Wire toggle text update
  const plToggle = document.getElementById('cfg-profitlock');
  if (plToggle) {
    plToggle.addEventListener('change', () => {
      document.getElementById('cfg-profitlock-text').textContent = plToggle.checked ? 'Enabled' : 'Disabled';
    });
  }
...
// ── Config save / reset ──
const DEFAULT_CONFIG = { ...bot.config };

window.applyBotConfig = function() {
  if (bot.running) {
    showToast('Stop the bot before changing config!', 'error');
    return;
  }

  bot.config.productType = document.getElementById('cfg-product').value;
  bot.config.quantityPerLot = parseInt(document.getElementById('cfg-lot').value) || 50;
  bot.config.optionStrikeDiff = parseInt(document.getElementById('cfg-strike').value) || 50;
  bot.config.profitEntryThreshold = parseFloat(document.getElementById('cfg-entry').value) || 0.07;
  bot.config.dailyMaxLoss = parseInt(document.getElementById('cfg-maxloss').value) || 1000;
  bot.config.initialProfitLockTrigger = parseInt(document.getElementById('cfg-locktrigger').value) || 600;
  bot.config.profitLockBuffer = parseInt(document.getElementById('cfg-lockbuf').value) || 100;
  bot.config.marketCloseTime = document.getElementById('cfg-close').value || '15:15';
  bot.config.firstCandleStartTime = document.getElementById('cfg-candlestart').value || '09:15';
  bot.config.firstCandleEndTime = document.getElementById('cfg-candleend').value || '09:18';
  bot.config.dynamicProfitLockEnabled = document.getElementById('cfg-profitlock').checked;

  // Sim speed
  bot._simSpeed = parseInt(document.getElementById('cfg-speed').value) || 1000;

  // Persist to localStorage
  localStorage.setItem('algobeat_bot_config', JSON.stringify(bot.config));
  localStorage.setItem('algobeat_bot_speed', bot._simSpeed);

  showToast('Configuration saved! Start the bot to apply.', 'success');
};

window.resetBotConfig = function() {
  if (bot.running) {
    showToast('Stop the bot before resetting config!', 'error');
    return;
  }

  Object.assign(bot.config, DEFAULT_CONFIG);
  bot._simSpeed = 1000;
  localStorage.removeItem('algobeat_bot_config');
  localStorage.removeItem('algobeat_bot_speed');

  // Re-render page to reflect defaults
  const main = document.getElementById('main-content');
  if (main) renderLiveBotPage(main);
...
(function loadSavedConfig() {
  try {
    const saved = localStorage.getItem('algobeat_bot_config');
    if (saved) Object.assign(bot.config, JSON.parse(saved));
    const speed = localStorage.getItem('algobeat_bot_speed');
    if (speed) bot._simSpeed = parseInt(speed);
  } catch (e) { /* ignore */ }
})();
